...
void GameApp::UpdateScene(float dt)
{
	...
	if (m_MouseTracker.leftButton == Mouse::ButtonStateTracker::PRESSED)
	{
		...
		m_Box[i].GetTransform().SetPosition(m_pCamera->GetPosition().x,-2.0f, m_pCamera->GetPosition().z+5.0f);
		//多次创建
		i++;
		if (i >= 3)
			i = 0;
...
void GameApp::DrawScene()
{
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		std::wstring text = L"Esc退出 摄像机模式:Q、第一人称 E、自由视角  \n"
			L"鼠标移动控制视野 W/S/A/D移动\n"
			L"切换天空盒: 1-颠倒世界 2-日落 3-沙漠\n"
			L"当前天空盒: ";

		std::wstring text1 = L"当前拾取物体: " + m_PickedObjStr;

		m_pd2dRenderTarget->DrawTextW(text1.c_str(), (UINT32)text1.length(), m_pTextFormat.Get(),
			D2D1_RECT_F{800.0f, 0.0f, 1600.0f, 200.0f }, m_pColorBrush.Get());

		std::wstring text2 = L"左键创建(3个上限）\n"
			L"右键销毁(指定)";

		m_pd2dRenderTarget->DrawTextW(text2.c_str(), (UINT32)text2.length(), m_pTextFormat.Get(),
			D2D1_RECT_F{ 800.0f, 15.0f, 1600.0f, 200.0f }, m_pColorBrush.Get());

		switch (m_SkyBoxMode)
		{
		case SkyBoxMode::Daylight: text += L"颠倒世界"; break;
...
bool GameApp::InitResource()
{
	...
	model.SetMesh(m_pd3dDevice.Get(), Geometry::CreatePlane(XMFLOAT2(20.0f, 20.0f), XMFLOAT2(5.0f, 5.0f)));
	model.modelParts[0].material.ambient = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
	...
	camera->LookTo(XMFLOAT3(0.0f, 0.0f, -5.0f), XMFLOAT3(0.0f, 0.0f, 1.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
	...
	// 设置调试对象名
	//
	m_Box[0].SetDebugObjectName("Box");
	m_Box[1].SetDebugObjectName("Box1");
	m_Box[2].SetDebugObjectName("Box2");
